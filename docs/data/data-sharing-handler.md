...
- If the application does not clearly show the above, follow the instructions under [Application is incomplete and should not be amended](#application-is-incomplete-and-should-not-be-amended) if this application would never be approved (e.g. from a region we're not allowed to share with)
  or [Application is incomplete but can be amended](#application-is-incomplete-but-can-be-amended) if the application is just missing enough information.
...
#### Application is incomplete and should not be amended
...
#### Application is incomplete but can be amended
...
The automated application management system will create a link and password for the NextCloud repository and automatically send them via email to the recipient scientist (or applicant). This link is valid for 14 days.
